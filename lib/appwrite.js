...
export const createPreHospitalCare = async (patientInfo) => {
  try {
    const newPatientInfo = await databases.createDocument(
      ...
      ID.unique(),
      {
        ambulanceStation: patientInfo.ambulanceStation,
        shiftCode: patientInfo.shiftCode,
...
export const createPreBillingForm= async (preBilling) => {
  try {
    const newPatientInfo = await databases.createDocument(
      ...
      ID.unique(),
      {
        ambulanceStationNumber: preBilling.ambulanceStationNumber,
        patientFirstName: preBilling.patientFirstName,
        patientLastName: preBilling.patientLastName,
      }
    );
    return newPatientInfo;
  } catch (error) {
